...
function Modules.Rejoin()
    ...
    pcall(function()
        TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, Modules.Player)
    end)
end
function Modules.ServerHop()
    if not TeleportService or not Modules.Player then
        return
    end
...
function Modules.ShootMurdererOnce()
    ...
    if not (Modules.FindMurderer or Modules.FindSheriffThatsNotMe) then
        return
    end
...
do
    function Modules.KillAll()
        if not Modules.Backpack or not Modules.Character or not Modules.Humanoid then
            return
        end
...
do
    ...
    function Modules.ToggleNotifyGunDropped(enable)
        ...
        _notifyConn = AddConnection(Workspace.DescendantAdded:Connect(function(desc)
            if not Modules.Config.NotifyGunDropped then
                return
            end
...
do
    ...
    function Modules.ExecuteThrowAtNearest()
        ...
        if not knife or not knife:FindFirstChild('Throw') then
            return
        end
...
do
    ...
    function Modules.UpdateRoleNotify()
        if roleMonitorTask then
            task.cancel(roleMonitorTask)
...
do
    function Modules.CreateGuiButton(id, settings, callback)
        if Modules.MobileButtons[id] then
            if Modules.MobileButtons[id].Parent then
                Modules.MobileButtons[id]:Destroy()
            end
        ...
        btn.InputChanged:Connect(function(input)
            if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) and dragging then
                if Modules.Config.ButtonsLocked then
                    return
                end
...
do
    ...
    local function ToggleESP_System(enabled)
        if enabled then
            ...
            if not Modules.Config.EspPlayers and not Modules.Config.ShowNames then
                ...
                for _, p in ipairs(Players:GetPlayers())do
                    if p.Character then
                        if p.Character:FindFirstChild('VisionHighlight') then
                            p.Character.VisionHighlight:Destroy()
                        end
                        if p.Character:FindFirstChild('Head') and p.Character.Head:FindFirstChild('NameESP') then
                            p.Character.Head.NameESP:Destroy()
                        end
                    end
                end
            end
    ...
    local function RemoveGunESP()
        for _, obj in ipairs(Workspace:GetDescendants())do
            if obj.Name == 'GunDrop' then
                if obj:FindFirstChild('VisionGunHighlight') then
                    obj.VisionGunHighlight:Destroy()
                end
                if obj:FindFirstChild('VisionGunLabel') then
                    obj.VisionGunLabel:Destroy()
                end
            end
        end
    end
    local function AddGunESP(gun)
        ...
        if gun:FindFirstChild('VisionGunHighlight') then
            gun.VisionGunHighlight:Destroy()
        end
        if gun:FindFirstChild('VisionGunLabel') then
            gun.VisionGunLabel:Destroy()
        end
...
do
    ...
    buttonsSection:Toggle({
        ...
        Callback = function(enabled)
            Modules.CreateGuiButton('FlingMurderer', {
                Enabled = enabled,
                Type = 'Text',
                Text = 'Fling Murderer',
    ...
    buttonsSection:Toggle({
        ...
        Callback = function(enabled)
            Modules.CreateGuiButton('FlingSheriff', {
                Enabled = enabled,
                Type = 'Text',
                Text = 'Fling Sheriff',
                TextColor = Color3.fromRGB(255, 255, 255),
    ...
    buttonsSection:Toggle({
        ...
        Callback = function(enabled)
            Modules.CreateGuiButton('FlingHero', {
                Enabled = enabled,
                Type = 'Text',
                Text = 'Fling Hero',
                TextColor = Color3.fromRGB(255, 255, 255),
    ...
    buttonsSection:Toggle({
        ...
        Callback = function(enabled)
            Modules.CreateGuiButton('ThrowKnife', {
                Enabled = enabled,
                Type = 'Text',
                Text = 'Throw Knife',
            }, Modules.ExecuteThrowAtNearest)
    ...
    buttonsSection:Toggle({
        ...
        Callback = function(enabled)
            Modules.CreateGuiButton('GrabGun', {
                Enabled = enabled,
                Type = 'Text',
                Text = 'Grab Gun',
            }, Modules.GrabGun)
    ...
    buttonsSection:Toggle({
        ...
        Callback = function(enabled)
            Modules.CreateGuiButton('Teleport', {
                Enabled = enabled,
                Type = 'Text',
    ...
    buttonsSection:Toggle({
        ...
        Callback = function(enabled)
            Modules.CreateGuiButton('TeleportLobby', {
                Enabled = enabled,
                Type = 'Text',
...
do
    ...
    TeleportsSection.Roles:Button({
        ...
        Callback = function()
            ...
            if murderer then
                Modules.TeleportToPlayer(murderer)
            end
        end,
    })
    TeleportsSection.Roles:Button({
        ...
        Callback = function()
            ...
            if sheriff then
                Modules.TeleportToPlayer(sheriff)
            end
        end,
    })
    TeleportsSection.Roles:Button({
