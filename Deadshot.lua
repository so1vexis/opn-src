...
local _uiOk, _uiErr = pcall(function()
    UI = loadstring(game:HttpGet('https://raw.githubusercontent.com/tsBelrux/dsui/refs/heads/main/ontop.lua'))()
end)

if not _uiOk or not UI then
    warn('[DS] UI Library hatas\u{131}: ' .. tostring(_uiErr))
...
local C_RED = CR(220, 20, 60)
local C_VIO = CR(138, 43, 226)
local C_CYAN = CR(0, 200, 255)
local C_GOLD = CR(255, 215, 0)
local C_GREEN = CR(0, 255, 127)
local C_BLUE = CR(30, 144, 255)
local C_WHITE = CR(255, 255, 255)
local C_GREY = CR(170, 170, 180)
...
_S.auraConn = nil
_S.throwAura = false
_S.throwAuraRange = 30
_S.throwConn = nil
_S.aimlockEnabled = false
_S.aimlockActive = false
_S.aimlockTarget = nil
_S.aimlockConn = nil
_S.aimlockGui = nil
_S.flickEnabled = false
_S.shotBtnGui = nil
_S.dualGun = false
_S.dualKnife = false
_S.hitbox = false
_S.hitboxSize = 5
_S.hitboxColor = C_RED
_S.hitboxAdorn = {}
_S.hitboxConn = nil
_S.fly = false
_S.flySpeed = 50
...
_S.noclip = false
_S.speedGlitch = false
_S.speedGlitchConn = nil
_S.bunny = false
_S.infJump = false
_S.ws = {
    Val = 16,
...
_S.speedBypass = false
_S.speedBypassVal = 9
_S.godMode = false
_S.espHL = false
_S.espBillboard = false
_S.espFilter = 'All'
_S.invis = false
_S.invisAnim = nil
_S.invisHB = nil
...
_S.coinsCollected = 0
_S.smartFarm = false
_S.autoFlingMurderer = false
_S.bjEnabled = false
_S.bjOnCD = false
_S.bjDebounce = false
_S.bjAutoGet = false
_S.bjJustSpawned = false
_S.bjGui = nil
_S.bjBtn = nil
_S.bjSize = 68
_S.ninjaEnabled = false
_S.ninjaCD = false
_S.ninjaOrigSpeed = 16
_S.selPlayer = nil
_S.origCamSubject = Camera.CameraSubject
_S.loopTP = false
_S.loopFling = false
_S.orbit = false
_S.orbitConn = nil
_S.deathNotify = false
_S.deathConn = nil
_S.flingAura = false
_S.clickFling = false
_S.antiFling = false
...
_S.killCount = 0
_S.roundTimer = 0
_S.roundTimerConn = nil
_S.crosshair = false
_S.crosshairGui = nil
_S.streamingMode = false
_S.streamConns = {}
_S.lastRole = ''
_S.roleToast = true
_S.dsToggleGui = nil
...
local AutoConfig = {
    BulletSpeed = 2500,
    MaxDistance = 4000,
    BasePrediction = 0.125,
    PingMultiplier = 0.6,
    DistanceMultiplier = 0.00035,
    SpeedMultiplier = 0.002,
    JumpCompensation = 0.85,
}
local TargetData = {
    LastPos = nil,
    LastTime = 0,
...
local function UpdatePingAvg()
    ...
    if ok then
        ...
        table.insert(PingData.Samples, ps)

        if #PingData.Samples > PingData.MaxSamples then
            table.remove(PingData.Samples, 1)
        end

        local sum = 0

        for _, s in ipairs(PingData.Samples)do
            sum = sum + s
        end

        PingData.Average = sum / #PingData.Samples
        PingData.Current = ps
    end
end
...
local function CalculateAutoPrediction(tRoot, mRoot)
    ...
    if hum then
        ...
        if st == Enum.HumanoidStateType.Jumping then
            jumpF = AutoConfig.JumpCompensation
    ...
    if hum and hum:GetState() == Enum.HumanoidStateType.Freefall then
        pred = pred - Vector3.new(0, Workspace.Gravity * t * t * 0.08, 0)
    end
    if _S.saSharp then
        pred = pred + Vector3.new(0, 1.2, 0)
    end

    return pred
end
local function Notif(t, c, d)
    UI.Notify(t, c or '', d or 3)
end
...
local function GetRole(p)
    ...
    if ch:FindFirstChild('Knife') or (bp and bp:FindFirstChild('Knife')) then
        return 'Murderer'
    end
    if ch:FindFirstChild('Gun') or (bp and bp:FindFirstChild('Gun')) or ch:FindFirstChild('Revolver') or (bp and bp:FindFirstChild('Revolver')) then
        return 'Sheriff'
    end
...
local function RoleColor(r)
    return r == 'Murderer' and C_RED or r == 'Sheriff' and C_BLUE or C_GREEN
end
...
local function GetSheriff()
    for _, p in ipairs(Players:GetPlayers())do
        if p ~= LocalPlayer and p.Character then
            ...
            if p.Character:FindFirstChild('Gun') or (bp and bp:FindFirstChild('Gun')) or p.Character:FindFirstChild('Revolver') or (bp and bp:FindFirstChild('Revolver')) then
                return p
            end
        end
    end
...
local function IsSheriff()
    ...
    return (bp and (bp:FindFirstChild('Gun') or bp:FindFirstChild('Revolver'))) or (ch and (ch:FindFirstChild('Gun') or ch:FindFirstChild('Revolver')))
end
...
local function CreateDSToggleBtn()
    if _S.dsToggleGui then
        _S.dsToggleGui:Destroy()
    end
    ...
    TweenService:Create(dot, TweenInfo.new(0.9, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {BackgroundTransparency = 0.6}):Play()
    TweenService:Create(stroke, TweenInfo.new(1.4, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
        Color = CR(200, 100, 255),
        Transparency = 0.6,
    }):Play()
    MakeDraggable(cont)
...
local function CreateCrosshair()
    if _S.crosshairGui then
        _S.crosshairGui:Destroy()
    end
    ...
    Line(18, 2, -14, 0, C_WHITE)
    Line(18, 2, 14, 0, C_WHITE)
    Line(2, 18, 0, -14, C_WHITE)
    Line(2, 18, 0, 14, C_WHITE)
    ...
    TweenService:Create(dot, TweenInfo.new(0.6, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {BackgroundTransparency = 0.5}):Play()

    _S.crosshairGui = sg
end
local function CreateKillHUD()
    if _S.killHudGui then
        _S.killHudGui:Destroy()
    end
    ...
    f.Size = UDim2.fromOffset(110, 48)
    f.Position = UDim2.new(0, 8, 0, 120)
    f.BackgroundColor3 = C_DARK
    f.BackgroundTransparency = 0.08
    f.BorderSizePixel = 0
    ...
    kV.Size = UDim2.new(1, -10, 0.5, 0)
    kV.Position = UDim2.new(0, 6, 0.48, -2)
    kV.BackgroundTransparency = 1
    kV.Text = '0'
    kV.Font = Enum.Font.GothamBold
    kV.TextSize = 20
    kV.TextColor3 = C_WHITE
    kV.TextXAlignment = Enum.TextXAlignment.Left

    RunService.Heartbeat:Connect(function()
        kV.Text = tostring(_S.killCount)
    end)
...
local function CreateRoundTimerHUD()
    if _S.roundTimerGui then
        _S.roundTimerGui:Destroy()
    end

    local sg = Instance.new('ScreenGui', CoreGui)

    sg.Name = 'DSRoundTimer'
    sg.ResetOnSpawn = false

    local f = Instance.new('Frame', sg)

    f.Size = UDim2.fromOffset(110, 48)
    f.Position = UDim2.new(0, 8, 0, 174)
    f.BackgroundColor3 = C_DARK
    f.BackgroundTransparency = 0.08
    f.BorderSizePixel = 0
    Instance.new('UICorner', f).CornerRadius = UDim.new(0.2, 0)
    ...
    t1.Font = Enum.Font.GothamBold
    t1.TextSize = 10
    t1.TextColor3 = C_GOLD
    t1.TextXAlignment = Enum.TextXAlignment.Left

    local rV = Instance.new('TextLabel', f)

    rV.Size = UDim2.new(1, -10, 0.5, 0)
    rV.Position = UDim2.new(0, 6, 0.48, -2)
...
local function ShowRoleToast(role)
    if not _S.roleToast then
        return
    end

    local color = RoleColor(role)
    local icon = role == 'Murderer' and '\u{1f52a}' or role == 'Sheriff' and '\u{1f52b}' or '\u{1f607}'
    local sg = Instance.new('ScreenGui', CoreGui)
    ...
    TweenService:Create(f, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Position = UDim2.new(0.5, -140, 0.07, 0),
    }):Play()
    task.delay(2.8, function()
        TweenService:Create(f, TweenInfo.new(0.3), {
            Position = UDim2.new(0.5, -140, -0.12, 0),
        }):Play()
        task.wait(0.35)
        sg:Destroy()
    end)
...
task.spawn(function()
    ...
    if RoundStart then
        RoundStart.OnClientEvent:Connect(function()
            ...
            if _S.roundTimerConn then
                _S.roundTimerConn:Disconnect()
            end

            _S.roundTimerConn = RunService.Heartbeat:Connect(function(dt)
                if _S.roundActive then
                    _S.roundTimer += dt
                end
            end)
...
local function CreateAimlockBtn()
    if _S.aimlockGui then
        _S.aimlockGui:Destroy()
    end
    ...
    cont.Size = UDim2.new(0, 68, 0, 68)
    cont.Position = UDim2.new(0, 12, 0.5, -34)
    cont.BackgroundTransparency = 1

    local btn = Instance.new('TextButton', cont)

    btn.Size = UDim2.new(1, 0, 1, 0)
    btn.BackgroundColor3 = CR(12, 6, 24)
    ...
    bg2.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, C_VIO),
    ...
    TweenService:Create(stroke2, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
        Color = CR(255, 200, 100),
        Transparency = 0.6,
    }):Play()

    local cross2 = Instance.new('TextLabel', btn)

    cross2.Size = UDim2.new(1, 0, 0.55, 0)
    cross2.Position = UDim2.new(0, 0, 0.1, 0)
    cross2.BackgroundTransparency = 1
    cross2.Text = '\u{2295}'
    cross2.TextSize = 26
    cross2.TextColor3 = C_WHITE
    cross2.Font = Enum.Font.GothamBold

    local statLbl = Instance.new('TextLabel', btn)

    statLbl.Size = UDim2.new(1, 0, 0.28, 0)
    statLbl.Position = UDim2.new(0, 0, 0.7, 0)
    statLbl.BackgroundTransparency = 1
    statLbl.Text = 'OFF'
    statLbl.TextSize = 9
    statLbl.TextColor3 = C_GREY
            ...
            if p ~= LocalPlayer and p.Character then
                ...
                if p.Character:FindFirstChild('Knife') or (bp and bp:FindFirstChild('Knife')) then
                    ...
                    if hrp then
                        ...
                        if vis then
                            local dd = (Vector2.new(sp.X, sp.Y) - c).Magnitude
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if _S.aimlockActive then
            ...
            if _S.aimlockTarget then
                statLbl.Text = 'ON'
                statLbl.TextColor3 = C_GREEN
                cross2.TextColor3 = C_RED
                _S.aimlockConn = RunService.RenderStepped:Connect(function()
                    if not _S.aimlockTarget or not _S.aimlockTarget.Character then
                        ...
                        if _S.aimlockConn then
                            _S.aimlockConn:Disconnect()
...
local function CreatePredESP()
    if _S.saPredESP then
        _S.saPredESP:Destroy()
    end
    if _S.saPredPart then
        _S.saPredPart:Destroy()
    end

    _S.saPredPart = Instance.new('Part')
    _S.saPredPart.Name = 'DSPred'
    _S.saPredPart.Size = Vector3.new(2, 2, 2)
    _S.saPredPart.Transparency = 1
    _S.saPredPart.Anchored = true
    _S.saPredPart.CanCollide = false
    _S.saPredPart.Parent = workspace
    _S.saPredESP = Instance.new('BoxHandleAdornment')
    _S.saPredESP.Adornee = _S.saPredPart
    _S.saPredESP.Size = Vector3.new(2.4, 2.4, 2.4)
    _S.saPredESP.Color3 = CR(0, 255, 140)
    _S.saPredESP.Transparency = 0.3
    _S.saPredESP.AlwaysOnTop = true
    _S.saPredESP.ZIndex = 10
    _S.saPredESP.Parent = _S.saPredPart
end
local function UpdatePredESP(pos)
    if not _S.saShowPred or not _S.saPredESP then
        if _S.saPredESP then
            _S.saPredESP.Adornee = nil
        end
    ...
    _S.saPredPart.CFrame = CFrame.new(pos)
    _S.saPredESP.Adornee = _S.saPredPart
end
...
local function GetSATarget()
    ...
    for _, p in ipairs(Players:GetPlayers())do
        ...
        if not (p.Character:FindFirstChild('Knife') or (bp and bp:FindFirstChild('Knife')) or p.Character:FindFirstChild('Revolver') or (bp and bp:FindFirstChild('Revolver'))) then
            continue
        end
...
local function SAShoot()
    ...
    if _S.saWall and not HasClearShot(target) then
        if not already then
            gun.Parent = LocalPlayer.Backpack
        end
    ...
    pcall(function()
        remote:FireServer(CFrame.new(mr.Position), CFrame.new(aimPos))
    end)
    task.wait(0.05)
...
RunService.RenderStepped:Connect(function()
    if not _S.saShowPred or not _S.saEnabled then
        UpdatePredESP(nil)
    ...
    if t and t.Character then
        ...
        if hrp and mr then
            UpdatePredESP(CalculateAutoPrediction(hrp, mr))
        else
            UpdatePredESP(nil)
        end
    else
...
local function FlickShot()
    if UserInputService.MouseBehavior ~= Enum.MouseBehavior.LockCenter then
        Notif('Shift Lock Off', 'Enable it first!', 2)
    ...
    if h2 then
        ...
        if st == Enum.HumanoidStateType.Jumping or st == Enum.HumanoidStateType.Freefall then
            jumpPred = Vector3.new(0, vel.Y * ping * 2, 0)
        end
    end

    local targetPos = head.Position + vel * ping * 2 + jumpPred
    local conn
    ...
    conn = RunService.RenderStepped:Connect(function()
        ...
        if shoot then
            ...
            if rh then
                pcall(function()
                    shoot:FireServer(CFrame.new(rh.Position), CFrame.new(targetPos))
                end)
            end
        end
        ...
        task.spawn(function()
            ...
            for i = 0, 1, 0.2 do
                Camera.CFrame = Camera.CFrame:Lerp(origCam, i)
...
local function CreateShotBtn()
    if _S.shotBtnGui then
        _S.shotBtnGui:Destroy()
    end
    ...
    cont.Size = UDim2.new(0, 110, 0, 110)
    cont.Position = UDim2.new(0.88, 0, 0.58, 0)
    cont.AnchorPoint = Vector2.new(0.5, 0.5)
    cont.BackgroundTransparency = 1

    local ring = Instance.new('Frame', cont)
    ...
    ringImg.Image = 'rbxassetid://6764432293'
    ringImg.ImageColor3 = C_VIO

    local btn = Instance.new('TextButton', cont)

    btn.Size = UDim2.new(0, 82, 0, 82)
    btn.Position = UDim2.new(0.5, 0, 0.5, 0)
    btn.AnchorPoint = Vector2.new(0.5, 0.5)
    btn.BackgroundColor3 = CR(10, 6, 20)
    btn.BackgroundTransparency = 0.05
    btn.Text = ''
    btn.AutoButtonColor = false
    Instance.new('UICorner', btn).CornerRadius = UDim.new(0.5, 0)
    ...
    cross.Position = UDim2.new(0.5, 0, 0.42, 0)
    cross.AnchorPoint = Vector2.new(0.5, 0.5)
    cross.BackgroundTransparency = 1

    local function MakeLine(sz, pos)
        local l = Instance.new('Frame', cross)
    ...
    local dot2 = MakeLine(UDim2.new(0, 7, 0, 7), UDim2.new(0.5, 0, 0.5, 0))

    dot2.BackgroundColor3 = CR(255, 60, 60)

    local lbl = Instance.new('TextLabel', btn)

    lbl.Size = UDim2.new(1, 0, 0.22, 0)
    lbl.Position = UDim2.new(0, 0, 0.76, 0)
    lbl.BackgroundTransparency = 1
    lbl.Text = 'SHOOT'
    lbl.TextSize = 11
    lbl.TextColor3 = C_WHITE
    ...
    glowStroke.Color = CR(160, 80, 255)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if _S.flickEnabled then
            FlickShot()
        elseif _S.saEnabled then
            SAShoot()
        end

        lbl.Text = 'BANG!'
        lbl.TextColor3 = CR(255, 220, 80)
...
local function FindKnifeRemote(knife)
    ...
    for _, c in ipairs(knife:GetDescendants())do
...
local function StartKnifeSA()
    if _S.knifeConn then
        _S.knifeConn:Disconnect()
    end

    _S.knifeConn = UserInputService.InputBegan:Connect(function(inp, gpe)
        if gpe or not _S.knifeSA then
            return
        end
        if not (inp.KeyCode == Enum.KeyCode.E or inp.UserInputType == Enum.UserInputType.MouseButton2) then
            return
        end
        ...
        if remote then
            if rtype == 'event' then
                pcall(function()
                    remote:FireServer(CFrame.new(pred))
                end)
            else
                pcall(function()
                    remote:InvokeServer(CFrame.new(pred))
                end)
            end
        else
            pcall(function()
...
local function StartThrowAura()
    if _S.throwConn then
        _S.throwConn:Disconnect()
    end

    _S.throwConn = RunService.Heartbeat:Connect(function()
        if not _S.throwAura then
            return
        end
...
local function AutoKillAll()
    ...
    if not knife then
        ...
        if not knife then
            Notif('Kill All', 'B\u{131}\u{e7}ak yok!', 2)
    ...
    Notif('Kill All! \u{1f52a}', #targets .. ' oyuncu', 3)
    ...
    con = RunService.RenderStepped:Connect(function()
        if tick() - st > 3 then
            ...
            Notif('Kill All \u{2705}', #targets .. ' hedef', 3)
        ...
        for _, c in pairs(targets)do
            if c and c.Parent and c.PrimaryPart then
                ...
                c:SetPrimaryPartCFrame(mr.CFrame * CFrame.new(0, 0, -2))
            end
        end
    end)
end
                ...
                if weap and #clones == 0 then
                    ...
                    if handle then
                        ...
                        for _, v in pairs(handle:GetDescendants())do
                            if v:IsA('SpecialMesh') or v:IsA('Decal') or v:IsA('Texture') then
...
local function FlingPlayers(playerNames, flingAll)
    ...
    local function FlingPlayer(targetPlayer)
        ...
        if targetHum and targetHum.Sit and not flingAll then
            return
        end
        if targetHead then
        ...
        local function Teleport(part, offset, rotation)
            if not part or not localRoot then
                return
            end
                ...
                if targetHum then
                    local ok, _ = pcall(function()
                        if part.Velocity.Magnitude < 50 then
                            ...
                            Teleport(part, CFrame.new(0, 1.5, 0) + targetHum.MoveDirection * part.Velocity.Magnitude / 1.25, CFrame.Angles(math.rad(spin), 0, 0))
                            task.wait()
                            Teleport(part, CFrame.new(0, -1.5, 0) + targetHum.MoveDirection * part.Velocity.Magnitude / 1.25, CFrame.Angles(math.rad(spin), 0, 0))
                            task.wait()
                            Teleport(part, CFrame.new(2.25, 1.5, -2.25) + targetHum.MoveDirection * part.Velocity.Magnitude / 1.25, CFrame.Angles(math.rad(spin), 0, 0))
                            task.wait()
                            Teleport(part, CFrame.new(-2.25, -1.5, 2.25) + targetHum.MoveDirection * part.Velocity.Magnitude / 1.25, CFrame.Angles(math.rad(spin), 0, 0))
                            task.wait()
                        else
                            Teleport(part, CFrame.new(0, 1.5, targetHum.WalkSpeed), CFrame.Angles(math.rad(90), 0, 0))
                            task.wait()
                            Teleport(part, CFrame.new(0, -1.5, -targetHum.WalkSpeed), CFrame.Angles(0, 0, 0))
                            task.wait()
                        end
                    end)
                    ...
                    if targetPlayer.Character ~= targetChar then
                        break
                    end
        ...
        repeat
            if not localRoot or not getgenv().OldPos then
                break
            end
    ...
    if flingAll then
        ...
        for _, name in pairs(playerNames or {})do
            local tp = GetPlayerFromName(name)
...
local function MiniFling(target)
    if not target then
        return
    end
...
local function FlingMurderer()
    ...
    if m then
        Notif('Fling \u{1f32a}\u{fe0f}', m.Name, 2)
        task.spawn(function()
            FlingPlayers({
                m.Name,
            }, false)
...
local function FlingSheriff()
    ...
    if s then
        Notif('Sheriff Fling \u{1f32a}\u{fe0f}', s.Name, 2)
        task.spawn(function()
            FlingPlayers({
                s.Name,
            }, false)
...
local function GetGunNow()
    ...
    for _, v in ipairs(workspace:GetDescendants())do
        if v.Name == 'GunDrop' then
            pcall(function()
                firetouchinterest(tp, v, 0)
                task.wait()
                firetouchinterest(tp, v, 1)
            end)
    ...
    Notif('GUN', found and 'Gun picked up \u{1f52b}' or 'No GunDrop found!', 2)
end
        ...
        while _S.coinFarm do
            ...
            for _, coin in ipairs(coins)do
                ...
                if d > 150 then
                    ...
                    local tw = TweenService:Create(hrp, TweenInfo.new(d / 25, Enum.EasingStyle.Linear), {
                    ...
                    while tick() - t < d / 25 + 0.5 do
                        task.wait()

                        if not coin or not coin.Parent then
                            break
                        end
                        if (hrp.Position - coin.Position).Magnitude < 2 then
                            break
                        end
                    end
                ...
                pcall(function()
                    firetouchinterest(tp, coin, 0)
                    task.wait(0.02)
                    firetouchinterest(tp, coin, 1)
                end)
                task.wait(0.05)
            end
...
local function CreateBB(p)
    ...
    frame.BackgroundColor3 = CR(0, 0, 0)
...
local function UpdateESP()
    for _, p in ipairs(Players:GetPlayers())do
        ...
        if _S.espHL and p.Character then
            CreateHL(p)
        else
            RemoveHL(p)
        end
        if _S.espBillboard and p.Character then
            CreateBB(p)
        else
            RemoveBB(p)
        end
...
local function ClearESP()
    for p in pairs(_espHL)do
        RemoveHL(p)
    end
    for p in pairs(_espBB)do
        RemoveBB(p)
    end
end
...
task.spawn(function()
    while true do
        ...
        if _S.espHL or _S.espBillboard then
            UpdateESP()
        end
    end
end)
            ...
            if ch and _S.hitbox then
                ...
                if rp then
                    if adorn and adorn.Parent then
                        adorn.Size = Vector3.new(_S.hitboxSize, _S.hitboxSize, _S.hitboxSize)
                        adorn.Color3 = _S.hitboxColor
                    else
                        local na = Instance.new('BoxHandleAdornment')

                        na.Adornee = rp
                        na.Size = Vector3.new(_S.hitboxSize, _S.hitboxSize, _S.hitboxSize)
                        na.Color3 = _S.hitboxColor
                        na.Transparency = 0.4
                        na.ZIndex = 10
                        na.Parent = rp
                        _S.hitboxAdorn[p] = na
...
local function EnableInvis(char)
    ...
    if animator then
        if _S.invisABlock then
            _S.invisABlock:Disconnect()
        end

        _S.invisABlock = hum.AnimationPlayed:Connect(function(at)
            if at.Animation.AnimationId:match('%d+') ~= InvisAnimId then
                at:Stop()
            end
        end)

        if _S.invisHB then
            _S.invisHB:Disconnect()
        end
        ...
        _S.invisHB = RunService.Heartbeat:Connect(function()
            if _S.invis and not (_S.invisAnim and _S.invisAnim.IsPlaying) then
                ...
                anim.AnimationId = 'rbxassetid://' .. InvisAnimId
                _S.invisAnim = animator:LoadAnimation(anim)

                _S.invisAnim:Play()

                _S.invisAnim.TimePosition = 0.2

                _S.invisAnim:AdjustSpeed(0)
            end
        end)
    end
    if _S.invisCol then
        _S.invisCol:Disconnect()
    end
...
local function EnableSpeedGlitch()
    if _S.speedGlitchConn then
        _S.speedGlitchConn:Disconnect()
    end
    ...
    _S.speedGlitchConn = h2.StateChanged:Connect(function(_, new)
        if _S.speedGlitch and new == Enum.HumanoidStateType.Jumping then
            task.spawn(function()
                ...
                local o = h2.WalkSpeed

                h2.WalkSpeed = 250

                task.wait(0.4)

                if _S.speedGlitch then
                    h2.WalkSpeed = o
                end
            end)
        end
...
local function BJ_StartCD()
    ...
    if _S.bjBtn and _S.bjBtn.Parent then
        _S.bjBtn.Text = '\u{23f3}'
    end

    task.spawn(function()
        for i = 22, 1, -1 do
            if not _S.bjOnCD then
                break
            end
            if _S.bjBtn and _S.bjBtn.Parent then
                _S.bjBtn.Text = tostring(i)
            end
        ...
        if _S.bjBtn and _S.bjBtn.Parent then
            _S.bjBtn.Text = '\u{1f4a3}'
        end
    end)
end
local function FastBombJump()
    if _S.bjOnCD or _S.bjDebounce or _S.bjJustSpawned then
        return
    end
    ...
    if ok and bomb then
        ...
        if hrp then
            ...
            if rem then
                pcall(function()
                    rem:FireServer(CFrame.new(hrp.Position + (Camera.CFrame.LookVector * 5)), 50)
                end)
            end
...
local function NinjaStepActivate()
    ...
    _S.ninjaCD = true
    _S.ninjaOrigSpeed = hum.WalkSpeed
    hum.WalkSpeed = 50

    Notif('\u{26a1} Ninja Step!', '5 saniye h\u{131}z 50', 2)
    task.spawn(function()
        task.wait(5)
        ...
        if LocalPlayer.Character then
            ...
            if h2 then
                h2.WalkSpeed = _S.ninjaOrigSpeed
            end
        end

        _S.ninjaCD = false

        Notif('Ninja Step', 'H\u{131}z normal', 2)
    end)
end
local function CreateKillEffect(position)
    task.spawn(function()
        ...
        pe.Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0, CR(255, 60, 60)),
            ColorSequenceKeypoint.new(0.5, CR(255, 160, 0)),
            ColorSequenceKeypoint.new(1, CR(80, 0, 200)),
        }
        pe.LightEmission = 1
        pe.LightInfluence = 0
...
local function CreateCharTrail()
    ...
    trail.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, CR(180, 0, 255)),
        ColorSequenceKeypoint.new(0.5, CR(80, 200, 255)),
        ColorSequenceKeypoint.new(1, CR(255, 80, 180)),
    }
...
local function CreateKnifeTrail()
    ...
    a0.Name = 'DSKnifeTrailA0'

    local a1 = Instance.new('Attachment', handle)

    a1.Position = Vector3.new(0, -0.5, 0)
    a1.Name = 'DSKnifeTrailA1'

    local trail = Instance.new('Trail', handle)

    trail.Name = 'DSKnifeTrail'
    trail.Attachment0 = a0
    trail.Attachment1 = a1
    trail.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, CR(255, 20, 60)),
        ColorSequenceKeypoint.new(0.5, CR(255, 140, 0)),
        ColorSequenceKeypoint.new(1, CR(255, 255, 100)),
    }
    trail.Transparency = NumberSequence.new{
        NumberSequenceKeypoint.new(0, 0),
        NumberSequenceKeypoint.new(1, 1),
...
local function EnableStreamingMode()
    for _, c in pairs(_S.streamConns)do
        pcall(function()
            c:Disconnect()
        end)
    end
    ...
    if kc then
        table.insert(_S.streamConns, kc)
    end
            ...
            if h then
                local dc = h.Died:Connect(function()
                    if p.Character and p.Character:FindFirstChild('HumanoidRootPart') then
                        CreateKillEffect(p.Character.HumanoidRootPart.Position)
                    end
                end)

                table.insert(_S.streamConns, dc)
            end
        end
    end
...
local function DisableStreamingMode()
    ...
    if _S.streamGui then
        _S.streamGui:Destroy()

        _S.streamGui = nil
    end
end
local function SelectPlayer(name)
    if _S.deathConn then
        _S.deathConn:Disconnect()
    ...
    for _, p in ipairs(Players:GetPlayers())do
        if p.Name:lower():find(lower, 1, true) or p.DisplayName:lower():find(lower, 1, true) then
            ...
            Notif('Se\u{e7}ildi: ' .. p.DisplayName, '', 3)

            return
        end
    end

    Notif('Oyuncu bulunamad\u{131}', '', 2)
end

LocalPlayer.CharacterAdded:Connect(function(char)
    _S.char = char
    _S.hrp = char:WaitForChild('HumanoidRootPart', 5)
    _S.hum = char:WaitForChild('Humanoid', 5)
    _S.bjOnCD = false
    ...
    _S.bjJustSpawned = true

    task.delay(1, function()
        _S.bjJustSpawned = false
    end)

    if _S.bjAutoGet then
        task.wait(1.2)
        pcall(function()
            ReplicatedStorage.Remotes.Extras.ReplicateToy:InvokeServer('FakeBomb')
        end)
    ...
    if h then
        if not _S.ws.Locked then
            h.WalkSpeed = _S.ws.Val
        end
        if not _S.jp.Locked then
            h.JumpPower = _S.jp.Val
        end
    end
...
UserInputService.InputChanged:Connect(function(inp)
    local td = _touches[inp]

    if not td then
        return
    end
    if (inp.Position - td.sp).Magnitude > 10 then
        td.moved = true
    end
end)
...
QB_Gui.Name = 'DSQuickBtns'
QB_Gui.ResetOnSpawn = false
QB_Gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
QB_Gui.Parent = cloneref(LocalPlayer:WaitForChild('PlayerGui'))

local EditModeLabel = Instance.new('TextLabel', QB_Gui)

EditModeLabel.Size = UDim2.new(0, 400, 0, 42)
EditModeLabel.Position = UDim2.new(0.5, 0, 0.36, 0)
EditModeLabel.AnchorPoint = Vector2.new(0.5, 0.5)
EditModeLabel.BackgroundTransparency = 1
EditModeLabel.Visible = false
EditModeLabel.Font = Enum.Font.GothamBold
EditModeLabel.TextColor3 = Colors.Accent1
EditModeLabel.TextSize = 18
EditModeLabel.TextStrokeTransparency = 0.3

local function RefreshEditLabel()
    if _S.editMode and SelectedButton then
        EditModeLabel.Text = '\u{270f}  EDIT: ' .. SelectedButton.Name:upper()
...
local function MakeBtnDraggable(gui)
    ...
    gui.InputBegan:Connect(function(input)
        if not _S.editMode then
            return
        end
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if SelectedButton and SelectedButton ~= gui then
                SelectedButton.BackgroundColor3 = CR(20, 18, 30)
            end

            SelectedButton = gui
            gui.BackgroundColor3 = Colors.Accent1
...
local function SaveLayout()
    ...
    for _, btn in ipairs(CreatedButtons)do
        data[btn.Name] = {
            PX = btn.Position.X.Scale,
            PXO = btn.Position.X.Offset,
            PY = btn.Position.Y.Scale,
            PYO = btn.Position.Y.Offset,
            SX = btn.Size.X.Offset,
            SY = btn.Size.Y.Offset,
        }
    end
    ...
    Notif('Kaydedildi \u{2705}', 'Layout kaydedildi', 3)
end
local function LoadLayout()
    pcall(function()
        if not isfile or not isfile(SAVE_FILE) then
            return
        end
...
local function CreateQBtn(name, pos, text, callback)
    ...
    btn.Position = pos
    btn.AnchorPoint = Vector2.new(0.5, 0.5)
    btn.BackgroundColor3 = CR(20, 18, 30)
    ...
    RunService.RenderStepped:Connect(function(dt)
        grad.Rotation = (grad.Rotation + (160 * dt)) % 360
    end)
...
local function T(tab, title, default, cb)
    return Win:AddToggleTo(Win:GetPage(tab), title, default, cb)
end
local function S(tab, title, min, max, def, suf, cb)
    return Win:AddSliderTo(Win:GetPage(tab), title, min, max, def, suf, cb)
end
local function Btn(tab, text, icon, cb)
    return Win:AddButtonTo(Win:GetPage(tab), text, icon, cb)
end
local function Lbl(tab, text, col)
    return Win:AddLabel(tab, text, col)
end
local function Div(tab)
    return Win:AddDivider(tab)
end

Lbl('Main', '\u{2014} FOV', Colors.Accent3)
S('Main', 'Field of View', 30, 120, 70, '\u{b0}', function(v)
    Camera.FieldOfView = v
end)
Btn('Main', 'Reset FOV', '\u{21ba}', function()
    Camera.FieldOfView = 70

    Notif('FOV', '70\u{b0} Reset', 2)
end)
Div('Main')
Lbl('Main', '\u{2014} \u{1f52b} Sheriff / Combat', Colors.Accent3)
T('Main', 'Silent Aim (Pred.)', false, function(v)
    _S.saEnabled = v

    Notif(v and 'SA A\u{c7}IK \u{1f3af}' or 'SA KAPALI', '', 2)
end)
S('Main', 'Bullet Speed', 500, 5000, 2500, '', function(v)
    AutoConfig.BulletSpeed = v
end)
S('Main', 'Base Prediction (x100)', 1, 50, 12, '', function(v)
    AutoConfig.BasePrediction = v / 100
end)
S('Main', 'Ping Multiplier (x10)', 1, 20, 6, '', function(v)
    AutoConfig.PingMultiplier = v / 10
end)
T('Main', 'Sharp Shooter (Kafa +1.2)', false, function(v)
    _S.saSharp = v
end)
T('Main', 'Wall Check', false, function(v)
    _S.saWall = v
end)
T('Main', 'Prediction ESP (Ye\u{15f}il Kutu)', false, function(v)
    _S.saShowPred = v
...
T('Main', 'Flick Shot (Shift Lock)', false, function(v)
    _S.flickEnabled = v

    Notif(v and 'Flick ON' or 'OFF', '', 2)
end)
...
Btn('Main', 'Get Gun (GunDrop)', '\u{1f52b}', function()
    GetGunNow()
end)
Div('Main')
Lbl('Main', '\u{2014} \u{1f52a} Murderer / Knife', Colors.Danger)
Btn('Main', 'Kill All \u{1f52a}', '\u{2694}', function()
    AutoKillAll()
end)
Btn('Main', 'Kill Sheriff', '\u{1f3af}', function()
    local s = GetSheriff()
    ...
    if knife and s.Character then
        ...
        if tr and mr then
            ...
            Notif('Sheriff Killed!', s.Name, 2)
        end
    end
end)
T('Main', 'Knife Silent Aim', false, function(v)
    _S.knifeSA = v
    ...
    Notif(v and 'Knife SA ON' or 'OFF', '', 2)
end)
S('Main', 'Knife Yatay Pred.', 0, 500, 100, '', function(v)
    _S.knifeH = v / 100
end)
S('Main', 'Knife Dikey Pred.', 0, 500, 100, '', function(v)
    _S.knifeV = v / 100
end)
T('Main', 'Kill Aura', false, function(v)
    _S.killAura = v
    ...
    Notif(v and 'Kill Aura ON' or 'OFF', '', 2)
end)
S('Main', 'Kill Aura Range', 3, 60, 20, 's', function(v)
    _S.killAuraRange = v
end)
T('Main', 'Throw Knife Aura', false, function(v)
    _S.throwAura = v
    ...
    Notif(v and 'Throw Aura ON' or 'OFF', '', 2)
end)
S('Main', 'Throw Aura Range', 3, 80, 30, 's', function(v)
    _S.throwAuraRange = v
end)
T('Main', 'Dual Knife', false, function(v)
    _S.dualKnife = v

    if v then
        SpawnDualVisual('dualKnife', 'Knife')
    end
end)
T('Main', 'Dual Gun', false, function(v)
    _S.dualGun = v

    if v then
        SpawnDualVisual('dualGun', 'Gun')
    end
end)
Div('Main')
Lbl('Main', '\u{2014} \u{1f607} Innocent', Colors.Success)
T('Main', 'Invisibility', false, function(v)
    _S.invis = v

    if v then
        EnableInvis(LocalPlayer.Character)
    else
        DisableInvis(LocalPlayer.Character)
    end

    Notif(v and 'Invisible ON \u{1f47b}' or 'OFF', '', 2)
end)
T('Main', 'Hitbox Expander', false, function(v)
    _S.hitbox = v

    if v then
        if not _S.hitboxConn then
            _S.hitboxConn = RunService.Heartbeat:Connect(UpdateHitboxes)
        end
    else
        if _S.hitboxConn then
            _S.hitboxConn:Disconnect()

            _S.hitboxConn = nil
        end

        for _, a in pairs(_S.hitboxAdorn)do
            pcall(function()
                a:Destroy()
            end)
        end
...
S('Main', 'Hitbox Size', 1, 25, 5, 's', function(v)
    _S.hitboxSize = v
end)
Div('Main')
Lbl('Main', '\u{2014} \u{1f680} Fly', Colors.Accent4)
S('Main', 'Fly Speed', 10, 300, 50, '', function(v)
    _S.flySpeed = v
end)
T('Main', 'Fly (WASD+Space/Shift)', false, function(v)
    ...
    if v then
        ...
        local ib = UserInputService.InputBegan:Connect(function(inp, gp)
            if gp or inp.UserInputType ~= Enum.UserInputType.Keyboard then
                return
            end
...
Div('Main')
Lbl('Main', '\u{2014} \u{26a1} Movement', Colors.Warning)
S('Main', 'Walk Speed', 0, 250, 16, '', function(v)
    _S.ws.Val = v

    local h = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass('Humanoid')

    if h and not _S.ws.Locked then
        h.WalkSpeed = v
    end
end)
T('Main', 'Lock WalkSpeed', false, function(v)
    _S.ws.Locked = v
end)
S('Main', 'Jump Power', 0, 250, 50, '', function(v)
    _S.jp.Val = v

    local h = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass('Humanoid')

    if h and not _S.jp.Locked then
        h.JumpPower = v
    end
end)
T('Main', 'Lock JumpPower', false, function(v)
    _S.jp.Locked = v
end)
T('Main', 'Noclip', false, function(v)
    _S.noclip = v
end)
T('Main', 'Infinite Jump', false, function(v)
    _S.infJump = v
end)
T('Main', 'Bunny Hop', false, function(v)
    _S.bunny = v
end)
T('Main', 'Speed Glitch', false, function(v)
    _S.speedGlitch = v
...
T('Main', 'Speed Bypass (V tu\u{15f}u)', false, function(v)
    _S.speedBypass = v
end)
S('Main', 'Speed Bypass De\u{11f}eri', 1, 100, 9, '', function(v)
    _S.speedBypassVal = v
end)
T('Main', 'God Mode', false, function(v)
    _S.godMode = v
...
Lbl('ESP', '\u{2014} Player ESP', Colors.Accent3)
T('ESP', 'Highlight ESP', false, function(v)
    _S.espHL = v
    ...
    Notif(v and 'ESP A\u{c7}IK' or 'KAPALI', '', 2)
end)
T('ESP', 'Billboard (Name+Role+Dist)', false, function(v)
    _S.espBillboard = v
...
do
    ...
    Btn('ESP', 'ESP Filter: All \u{2192}', '\u{1f504}', function()
        fi = fi % #filterList + 1
        _S.espFilter = filterList[fi]
...
Div('ESP')
Lbl('ESP', '\u{2014} Role Expose', Colors.Warning)
...
Btn('ESP', 'Expose Murderer', '\u{1f4ac}', function()
    ExposeR('Knife', 'Murderer')
end)
Btn('ESP', 'Expose Sheriff', '\u{1f4ac}', function()
    ExposeR('Gun', 'Sheriff')
end)
...
Btn('ESP', 'Notify Murderer', '\u{1f514}', function()
    NotifRole('Knife', 'Murderer')
end)
Btn('ESP', 'Notify Sheriff', '\u{1f514}', function()
    NotifRole('Gun', 'Sheriff')
end)

do
    local ntM, ntS = nil, nil

    T('ESP', 'Auto Notify Murderer', false, function(v)
        if v then
            ntM = RunService.Heartbeat:Connect(function()
                if NotifRole('Knife', 'Murderer') and ntM then
                    ntM:Disconnect()
    ...
    T('ESP', 'Auto Notify Sheriff', false, function(v)
        if v then
            ntS = RunService.Heartbeat:Connect(function()
                if NotifRole('Gun', 'Sheriff') and ntS then
                    ntS:Disconnect()
...
Div('ESP')
Lbl('ESP', '\u{2014} Spectate', Colors.TextMuted)
Btn('ESP', 'Spectate Murderer', '\u{1f441}', function()
    local m = GetMurderer()

    if m and m.Character then
        local h = m.Character:FindFirstChildOfClass('Humanoid')
...
Btn('ESP', 'Spectate Sheriff', '\u{1f441}', function()
    local s = GetSheriff()

    if s and s.Character then
        local h = s.Character:FindFirstChildOfClass('Humanoid')
...
Btn('ESP', 'Stop Spectating', '\u{21a9}', function()
    if LocalPlayer.Character then
        local h = LocalPlayer.Character:FindFirstChildOfClass('Humanoid')
...
Lbl('Farm', '\u{2014} Coin Farm (Wraith Tabanl\u{131})', Colors.Accent3)
T('Farm', 'Auto Farm Coins', false, function(v)
    _S.coinFarm = v

    if v then
        WraithAutofarm()
        Notif('Coin Farm ON \u{1f4b0}', 'firetouchinterest', 2)
    else
        Notif('Coin Farm OFF', '', 2)
    end
end)
...
do
    ...
    Btn('Farm', 'Mode: Nearest \u{2192}', '\u{1f504}', function()
        mi = mi % #modeList + 1
        _S.coinMode = modeList[mi]

        Notif('Coin Mode', _S.coinMode, 2)
    end)
end

T('Farm', 'Smart Farm (Bag Full Action)', false, function(v)
    _S.smartFarm = v
end)
T('Farm', 'Auto Fling Murderer (Bag Full)', false, function(v)
    _S.autoFlingMurderer = v
end)
T('Farm', 'Spin (Coin Collect)', false, function(v)
    _S.spinEnabled = v

    if v then
        local rp = (LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()):WaitForChild('HumanoidRootPart')

        _S.spinConn = RunService.RenderStepped:Connect(function(_)
            if _S.spinEnabled and rp then
                rp.CFrame = rp.CFrame * CFrame.Angles(0, math.rad(_S.spinSpeed), 0)
            end
        end)
    else
        if _S.spinConn then
            _S.spinConn:Disconnect()

            _S.spinConn = nil
        end
    end
end)
S('Farm', 'Spin Speed', 1, 30, 5, '', function(v)
    _S.spinSpeed = v
end)
T('Farm', 'Anti-AFK', false, function(v)
    if v then
        _S.antiAFKConn = LocalPlayer.Idled:Connect(function()
            VirtualUser:Button2Down(Vector2.new(0, 0), Camera.CFrame)
            task.wait(10)
            VirtualUser:Button2Up(Vector2.new(0, 0), Camera.CFrame)
        end)
    elseif _S.antiAFKConn then
        _S.antiAFKConn:Disconnect()

        _S.antiAFKConn = nil
    end
end)
Div('Farm')
Lbl('Farm', '\u{2014} \u{1f4a3} Bomb Jump', Colors.Warning)
T('Farm', 'Bomb Jump (Dokunma/T\u{131}k)', false, function(v)
    _S.bjEnabled = v
end)
T('Farm', 'Ninja Step (5s Speed Boost)', false, function(v)
    _S.ninjaEnabled = v
end)
Btn('Farm', '\u{26a1} Ninja Step Aktifle\u{15f}tir', '\u{26a1}', function()
    NinjaStepActivate()
end)
T('Farm', 'Auto Get Fake Bomb', false, function(v)
    _S.bjAutoGet = v
end)
Btn('Farm', 'Get Fake Bomb', '\u{1f4a3}', function()
    pcall(function()
        ReplicatedStorage.Remotes.Extras.ReplicateToy:InvokeServer('FakeBomb')
    end)
    Notif('Bomb', 'Requested!', 2)
end)
Btn('Farm', 'Manuel Bomb Jump', '\u{1f4a5}', function()
    FastBombJump()
end)
T('Farm', 'BJ Button (Ekranda G\u{f6}ster)', false, function(v)
    if v then
        if _S.bjGui then
            _S.bjGui:Destroy()
        end

        _S.bjGui = Instance.new('ScreenGui')
        _S.bjGui.Name = 'DSBJGui'
        _S.bjGui.ResetOnSpawn = false
        _S.bjGui.Parent = LocalPlayer:WaitForChild('PlayerGui')
        _S.bjBtn = Instance.new('TextButton', _S.bjGui)
        _S.bjBtn.Text = '\u{1f4a3}'
        _S.bjBtn.TextSize = _S.bjSize / 3.5
        _S.bjBtn.Size = UDim2.new(0, _S.bjSize, 0, _S.bjSize)
        _S.bjBtn.Position = UDim2.new(0.5, -_S.bjSize / 2, 0.84, 0)
        _S.bjBtn.BackgroundColor3 = CR(25, 20, 40)
        _S.bjBtn.TextColor3 = C_WHITE
        _S.bjBtn.Font = Enum.Font.GothamBold
        _S.bjBtn.BackgroundTransparency = 0.2
        Instance.new('UICorner', _S.bjBtn).CornerRadius = UDim.new(1, 0)

        MakeDraggable(_S.bjBtn)
        _S.bjBtn.MouseButton1Click:Connect(function()
            if not _S.bjOnCD then
                FastBombJump()
            end
        end)
...
Lbl('Fling', '\u{2014} Role Fling', Colors.Danger)
Btn('Fling', 'Fling Murderer \u{1f32a}\u{fe0f}', '\u{1f32a}', function()
    FlingMurderer()
end)
Btn('Fling', 'Fling Sheriff \u{1f32a}\u{fe0f}', '\u{1f32a}', function()
    FlingSheriff()
end)
Btn('Fling', 'Fix Bug (OldPos)', '+', function()
    local mr = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild('HumanoidRootPart')

    if mr and getgenv().OldPos then
        mr.CFrame = getgenv().OldPos

        Notif('Fixed!', '', 2)
    else
        Notif('Kaydedilmi\u{15f} konum yok', '', 2)
    end
end)
Div('Fling')
Lbl('Fling', '\u{2014} Player Select', Colors.Accent3)
Btn('Fling', 'Oyuncu Listesi', '\u{1f4cb}', function()
    local names = {}
...
Btn('Fling', "Murderer'\u{131} Se\u{e7}", '\u{1f52a}', function()
    local m = GetMurderer()

    if m then
        SelectPlayer(m.Name)
    else
        Notif('Murderer yok', '', 2)
    end
end)
Btn('Fling', "Sheriff'i Se\u{e7}", '\u{1f52b}', function()
    local s = GetSheriff()
...
Div('Fling')
Lbl('Fling', '\u{2014} Se\u{e7}ili Oyuncu', Colors.Accent4)
Btn('Fling', 'TP to Selected', '\u{1f4cd}', function()
    if not _S.selPlayer then
        Notif('Oyuncu se\u{e7}', '', 2)
...
Btn('Fling', 'Fling Selected', '\u{1f32a}', function()
    if _S.selPlayer then
        MiniFling(_S.selPlayer)
    else
        Notif('Oyuncu se\u{e7}', '', 2)
    end
end)
T('Fling', 'Loop Fling', false, function(v)
    ...
    if v and _S.selPlayer then
        task.spawn(function()
            while _S.loopFling and _S.selPlayer and _S.selPlayer.Parent do
                MiniFling(_S.selPlayer)
                task.wait(0.5)
            end
        end)
...
T('Fling', 'Orbit (Etraf\u{131}nda D\u{f6}n)', false, function(v)
    _S.orbit = v

    if _S.orbitConn then
        _S.orbitConn:Disconnect()
    end
    if v and _S.selPlayer then
        local angle = 0

        _S.orbitConn = RunService.Heartbeat:Connect(function()
            if not _S.orbit then
                return
            end
            ...
            if mr and tr then
                angle = angle + 8
                mr.CFrame = CFrame.new(tr.Position) * CFrame.Angles(0, math.rad(angle), 0) * CFrame.new(10, 0, 0)
            end
        end)
    end
end)
T('Fling', 'Death Notify', false, function(v)
    ...
    if v and _S.selPlayer then
        ...
        if h then
            _S.deathConn = h.Died:Connect(function()
                Notif('\u{d6}ld\u{fc}!', _S.selPlayer.DisplayName, 4)
            end)
        end
    end
end)
Div('Fling')
Lbl('Fling', '\u{2014} Advanced Fling', Colors.Warning)
...
T('Fling', 'Click Fling', false, function(v)
    _S.clickFling = v
end)
T('Fling', 'Fling All', false, function(v)
    ...
    if v then
        task.spawn(function()
            while _S.flingAll do
                task.spawn(function()
                    FlingPlayers(nil, true)
                end)
                task.wait(0.5)
            end
        end)
    ...
    if v then
        for _, P in pairs(Players:GetPlayers())do
            if P ~= LocalPlayer then
                ...
                table.insert(_S.antiFlingConns, c)
            end
        end
    else
        for _, c in pairs(_S.antiFlingConns)do
            c:Disconnect()
        end

        table.clear(_S.antiFlingConns)
    end
end)
Lbl('Misc', '\u{2014} Visual', Colors.Accent3)
T('Misc', 'Fullbright', false, function(v)
    if v then
        Lighting.Brightness = 10
        Lighting.GlobalShadows = false
        Lighting.Ambient = CR(255, 255, 255)
    else
        Lighting.Brightness = 1
        Lighting.GlobalShadows = true
        Lighting.Ambient = CR(127, 127, 127)
    end
end)
T('Misc', 'Remove Fog', false, function(v)
...
Div('Misc')
Lbl('Misc', '\u{2014} Tools', Colors.TextMuted)

local function LS(url)
    task.spawn(function()
        pcall(function()
...
Btn('Misc', 'Inf Yield', '\u{1f527}', function()
    LS('https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source')
end)
Btn('Misc', 'Dex Explorer', '\u{1f50d}', function()
    LS('https://rawscripts.net/raw/Universal-Script-Classic-Dex-Explorer-21009')
end)
Btn('Misc', 'Remote Spy', '\u{1f441}', function()
    LS('https://raw.githubusercontent.com/78n/SimpleSpy/main/SimpleSpySource.lua')
end)
Btn('Misc', 'Rejoin', '\u{21ba}', function()
    game:GetService('TeleportService'):Teleport(game.PlaceId)
end)
Btn('Misc', 'Reset', '\u{1f480}', function()
    local h = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass('Humanoid')
...
Lbl('QB', '\u{2014} Quick Buttons', Colors.Accent1)
T('QB', 'T\u{fc}m Butonlar\u{131} G\u{f6}ster', false, function(v)
    for _, btn in ipairs(CreatedButtons)do
        btn.Visible = v
    end
end)
Div('QB')
Lbl('QB', '\u{2014} Edit Mode', Colors.Warning)
Btn('QB', '\u{270f} Edit Mode A\u{e7}/Kapat', '\u{270f}', function()
    _S.editMode = not _S.editMode

    if not _S.editMode and SelectedButton then
        SelectedButton.BackgroundColor3 = CR(20, 18, 30)
        SelectedButton = nil
    end

    RefreshEditLabel()
    Notif(_S.editMode and '\u{270f} Edit Mode ON' or 'Edit Mode OFF', '', 2)
end)
Btn('QB', '\u{1f4be} Save Layout', '\u{1f4be}', function()
    SaveLayout()
end)
Btn('QB', '\u{1f4c2} Load Layout', '\u{1f4c2}', function()
    LoadLayout()
    Notif('Y\u{fc}klendi \u{2705}', '', 2)
end)
S('QB', 'Buton Boyutu', 40, 120, 68, 'px', function(v)
    _S.bjSize = v

    for _, btn in ipairs(CreatedButtons)do
        btn.Size = UDim2.fromOffset(v, v)
    end
end)
Div('QB')
Lbl('QB', '\u{2014} Layout Presets', Colors.Accent3)

for _, preset in ipairs(QB_PRESETS)do
    local p = preset

    Btn('QB', '\u{1f4f1} ' .. p.name, '\u{1f4f1}', function()
        ApplyQBPreset(p)
    end)
end

Div('QB')
Lbl('QB', '\u{2014} Streaming Mode \u{1f3ac}', Colors.Accent4)
T('QB', 'Streaming Mode (Kill FX + Trail + Glow)', false, function(v)
    _S.streamingMode = v

    if v then
        EnableStreamingMode()
        Notif('\u{1f3ac} Streaming Mode ON', 'Kill FX + Trail', 3)
    else
        DisableStreamingMode()
        Notif('Streaming Mode OFF', '', 2)
    end
end)
    ...
    if v then
        ...
        if char then
            ...
            if knife then
                ...
                if handle then
                    for _, vv in pairs(handle:GetChildren())do
                        if vv.Name == 'DSKnifeTrail' then
                            vv:Destroy()
                        end
                    end
                end
...
Div('QB')
Lbl('QB', '\u{2014} HUD', Colors.Accent3)
...
T('QB', 'Round Timer HUD', false, function(v)
    if v then
        ...
        if _S.roundTimerGui then
            ...
            _S.roundTimerGui = nil
        end
    end
end)
T('QB', 'Role Toast (Round Start)', true, function(v)
    _S.roleToast = v
end)
Lbl('Settings', '\u{2014} UI', Colors.Accent3)
...
Div('Settings')
Lbl('Settings', '\u{2014} Config', Colors.TextMuted)
Btn('Settings', '\u{1f4be} Save Settings', '\u{1f4be}', function()
    Win.SaveSettings()
end)
Btn('Settings', '\u{1f4c2} Load Settings', '\u{1f4c2}', function()
    Win.LoadSettings()
end)
Div('Settings')
Lbl('Settings', '\u{2014} Info', Colors.TextMuted)
Btn('Settings', 'DeadShot Hub v33.0', '\u{2b50}', function()
    Notif('DeadShot Hub v33', 'ItsZorlux | ontop.lua UI | Wraith Fling+Farm', 4)
end)
Btn('Settings', "T\u{fc}m GUI'yi Temizle", '\u{1f5d1}', function()
    for _, sg in pairs(CoreGui:GetChildren())do
        if sg.Name:find('^DS') then
            pcall(function()
                sg:Destroy()
            end)
        end
    ...
    Notif('Temizlendi', 'Yeniden ba\u{15f}lat\u{131}n.', 2)
end)

local qSHOOT = CreateQBtn('SHOT_MRD', UDim2.new(0.82, 0, 0.55, 0), '\u{1f3af}\nSHOOT', function()
    SAShoot()
end)

CreateQBtn('FLING', UDim2.new(0.82, 0, 0.72, 0), '\u{1f32a}\u{fe0f}\nFLING', function()
    FlingMurderer()
end)
CreateQBtn('KILL_ALL', UDim2.new(0.82, 0, 0.38, 0), '\u{1f52a}\nKILL\nALL', function()
    AutoKillAll()
end)
CreateQBtn('ESP_BTN', UDim2.new(0.72, 0, 0.55, 0), '\u{1f441}\u{fe0f}\nESP', function()
    _S.espHL = not _S.espHL

    if not _S.espHL then
        ClearESP()
    end
    ...
    for _, b in ipairs(CreatedButtons)do
        if b.Name == 'ESP_BTN' then
            b.Text = _S.espHL and '\u{1f441}\u{fe0f}\nESP\nON' or '\u{1f441}\u{fe0f}\nESP'

            break
        end
    end
end)
CreateQBtn('NINJA', UDim2.new(0.72, 0, 0.72, 0), '\u{1f977}\nNINJA', function()
    if not _S.ninjaEnabled then
        _S.ninjaEnabled = true
    end

    NinjaStepActivate()
end)
CreateQBtn('INVIS', UDim2.new(0.72, 0, 0.38, 0), '\u{1f47b}\nINVIS', function()
    ...
    for _, b in ipairs(CreatedButtons)do
        if b.Name == 'INVIS' then
            b.Text = _S.invis and '\u{1f47b}\nINVIS\nON' or '\u{1f47b}\nINVIS'

            break
        end
    end
end)
CreateQBtn('GUN', UDim2.new(0.62, 0, 0.55, 0), '\u{1f52b}\nGET\nGUN', function()
    GetGunNow()
end)
CreateQBtn('BOMB', UDim2.new(0.62, 0, 0.72, 0), '\u{1f4a3}\nBOMB\nJUMP', function()
    FastBombJump()
end)
CreateQBtn('KNIFE_SA', UDim2.new(0.62, 0, 0.38, 0), '\u{1f3af}\nKNIFE\nSA', function()
    _S.knifeSA = not _S.knifeSA
    ...
    for _, b in ipairs(CreatedButtons)do
        if b.Name == 'KNIFE_SA' then
            b.Text = _S.knifeSA and '\u{1f3af}\nKNIFE\nSA ON' or '\u{1f3af}\nKNIFE\nSA'
...
do
    ...
    qSHOOT.MouseButton1Click:Connect(function()
        ...
        if now - lastTap < 0.35 then
            locked = not locked
            qSHOOT.Text = locked and '\u{1f3af}\nSHOOT\n\u{1f512}' or '\u{1f3af}\nSHOOT'

            local meta = _BtnMeta[qSHOOT]

            if meta then
                meta.st.Color = locked and C_GOLD or CR(200, 150, 255)
            end
...
pcall(LoadLayout)
CreateFPS()
Notif('DeadShot Hub v33 \u{1f3af}', 'DS butonu = A\u{e7}/Kapat | RightControl da \u{e7}al\u{131}\u{15f}\u{131}r\nWraith Fling+Farm | Kill HUD | Round Timer | DS Toggle', 6)
